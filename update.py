...
def update_offsets(offsets_h_path, offsets_ini_path):
    ...
    dumpFile.read(offsets_ini_path)

    # 读取 offset.h 文件内容
    with open(offsets_h_path, 'r') as headFile:
    ...
    # 将更新后的内容写回文件
    updated_content = '\n'.join(lines)
    with open(offsets_h_path, 'w') as headFile:
...
if __name__ == '__main__':
    args = sys.argv
    offset_h_path = "Offsets.hpp"
    offset_ini_path = "offsets.ini"
    if len(sys.argv) >= 3:
